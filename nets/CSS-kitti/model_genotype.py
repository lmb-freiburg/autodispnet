...
class DispNetCSS:
    ...
    def init_model(self):
        ...
        self.model_s2 = DispNetSGenotype(full_res=True, **self.net3_args)
...
def model_func(features, labels, mode, params):
    ...
    smult = 64/42
    net1_args = { 'C':42, 'feature_depth':1 , 'encoder_depth':6, 'decoder_depth':4, 'stem_multiplier': smult, 'genotype': nd.config['hyperparams']['genotype']}
    net2_args = { 'C':42, 'encoder_depth':7,  'decoder_depth':4, 'stem_multiplier': smult, 'genotype': nd.config['hyperparams']['genotype']}
    net3_args = { 'C':42, 'encoder_depth':7,  'decoder_depth':4, 'stem_multiplier': smult, 'genotype': nd.config['hyperparams']['genotype']}
